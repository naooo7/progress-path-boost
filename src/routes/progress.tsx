...
function ProgressPage() {
  ...
  return <Page><PageTitle title="Progress" subtitle="Every number below comes from your saved practice activity." />
    <div className="mb-6 flex w-fit gap-1 rounded-xl bg-secondary p-1">{options.map((item) => <Button key={item.value} type="button" size="sm" variant={range === item.value ? "secondaryElevated" : "ghost"} onClick={() => setRange(item.value)}>{item.label}</Button>)}</div>
